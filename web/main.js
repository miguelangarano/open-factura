async function getP12(path) {
  ...
  if (!response.ok) {
    throw new Error("Failed to get p12 file");
  }
...
async function getXML(path) {
  const response = await fetch(path);
  if (!response.ok) {
    throw new Error("Failed to get xml file");
  }
...
async function signXml(p12Path, p12Password, xmlPath) {
...
P12S.forEach(async (element) => {
  try {
    const signedXML = await signXml(
      "cert/" + element.fileName,
      element.password,
      "docs/factura.xml"
    );
